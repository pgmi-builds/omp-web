...
bundle patch 里挂载 omp-provider 的 `name` SHALL 等于 registry 包名 `@pgmi-builds/omp-webui`。系统 MUST NOT 使用任何旧别名（如 `dsh-omp-provider`）作为 mount name，除非同时以该别名 publish 到 registry。

#### Scenario: registry 安装后 loader 可解析

- **WHEN** user 从 registry 安装 `@pgmi-builds/omp-webui`（node_modules 下只有 `@pgmi-builds/omp-webui`）
...
- **WHEN** 从 registry 拉取 `@pgmi-builds/omp-webui` tarball
