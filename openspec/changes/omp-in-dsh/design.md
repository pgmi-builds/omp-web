...
- 仓库已重组：`apps/omp-webui`（现行生产桥接层，独立 `--profile omp` 实例，保持不动）+ `apps/omp-in-dsh`（本 change，共存型 plugin）。
- 桥接层经验全部来自 omp-webui：`OmpRpcClient.spawn()`（spawn `omp --mode rpc` 子进程）、`OmpAgent`（Dash `Agent` shim）、`replay`/`pairing`/`permission`/`omp-store`/`models`（store 扫描 + id 配对 + 审批映射 + /proc 排他 + 本机模型注册读取）。
- 上游源码事实（决策依据，均已核对源码）：
  ...
  - **`sessionPersistence` 单槽**：`agent-loop/src/index.ts:653-657` resume 硬依赖；omp-webui 的 `OmpUnionSessionPersistence` 以整服务替换方式占用该槽（仅适合单用途 profile）。
  - **OMP state 可官方重定向**：`PI_CODING_AGENT_DIR`（"Session storage directory"，oh-my-pi `help-extra.ts:57`）。v1 不启用，留作隔离模式开关。
  - **preset 可贡献**：DSH 原生支持自定义 preset（用户另一项目已验证 4 内建 + 1 自定义）；omp-webui 的 `SingleOmpPresetRoster` 是替换式 roster，不适用于共存场景。
...
- omp-webui 与其 `omp-plus` 服务零改动、可并存。
...
- 不动 omp-webui 生产形态；不做两 app 的共享库重构（见 D6）。

## Decisions

### D1: 子进程 RPC 承载 OMP core（沿用 omp-webui 形态）
**决策**：OMP core 以 `--mode rpc` 子进程运行，桥接层复用 omp-webui 的 rpc/agent/replay/pairing/permission/omp-store/models 模块（拷贝进 `apps/omp-in-dsh/src/bridge/`）。
**理由**：Bun 运行时耦合使进程内集成不可行；共享模式下桥接层与本机 state 完全同构于 omp-webui 生产形态，复用度 ~99%。
...
- preset/meta 判定为 `omp` → 走 OMP 分支（现 omp-webui `OmpProvider.createAgent/resume` 逻辑）；
...
- **seed 消毒**：apiproxy 以 `defaultModelSelection()` 作 seed 经 `ctx.agents.create` 流入 factory；原生分支检测 seed provider 带 `omp/` 前缀时替换为原生可用模型再委托 AgentLoop（防"omp 默认被持久化后，原生新会话因未知 provider 失败"）。OMP 分支忽略 seed（初始模型由 OMP 侧配置决定，沿用 omp-webui）。
- adapter 仅实现 `providerInfo/listModels/resolveModel`（读本机 `~/.omp/agent` 模型注册，omp-webui `models.ts` 原样）；`stream()` 抛 `UNSUPPORTED_STREAM`——OMP 生成永远发生在 OMP 子进程内。
- 已知限制继承 omp-webui：OMP 会话的 `current` 回显依赖 Dash 侧 `requestHeader().config`，OMP 内部模型与选择器显示可能不同步（记忆中的 v0.0.3 已知问题），首版沿用。
**备选**：静态常驻注册——暴露面更大，被否决；替换 llm 服务为包装器——需影响 apiproxy 的服务解析且仍无 per-session 上下文，否决。

### D5: v1 复用本机 OMP；隔离模式保留为参数开关
**决策**：
- spawn 目标 = PATH `omp`（或部署配置 `OMP_BIN` 绝对路径），state 共享本机 `~/.omp`——桥接层 `OMP_SESSIONS_ROOT`/`OMP_AGENT_DIR` 常量保持 omp-webui 原值，零参数化改动。
...
**理由**：用户决策——复用本机实例使实现/测试最简、TUI 保留、桥接复用 ~99%；"完全独立于原生安装"的原目标降级为后续可选模式。omp-webui 生产已验证共享模式（/proc 排他、TUI 会话互见）。
**代价**：本机 OMP 独立升级会漂移 plugin 兼容性——版本握手兜底；Web UI 会话写入 `~/.omp`（共享即有意为之，非泄漏）。

### D6: 代码组织 = omp-in-dsh 内嵌桥接副本，不做共享库
**决策**：`apps/omp-in-dsh/src/bridge/` 拷贝 omp-webui 桥接模块（共享模式下近原样，仅补版本握手）；omp-webui 零改动。
...
**理由**：`sessionPersistence` 与 factory 同为单槽；"沿用 DSH 基底 + OMP 自持"经复合路由同时满足。OMP 会话事实源仍是 OMP store（无副本日志），延续 omp-webui Phase C 结论。

### D8: workspace 归组与会话发现
**决策**：沿用 omp-webui 的 30s 周期 reconcile（`#reconcileWorkspaces`）+ 启动 warm pass，扫描根 = 本机 OMP store（`~/.omp/agent/sessions`），排除 OMP/DSH home 自身目录——与 omp-webui 完全一致（共享模式下无隔离根，原语义直接成立）。
**理由**：Web UI 之外创建的 OMP 会话（TUI）不经 apiproxy create 路径，需周期归组；omp-webui 生产已验证该机制。
...
- [OMP 会话 fork 不支持] → 沿用 omp-webui 边界：factory 边界明确拒绝（`parentSession` 检查已有）。
- [OMP 会话 `current` 回显依赖 Dash requestHeader，与 OMP 内部模型可能不同步] → 首版沿用 omp-webui 已知限制，文档标注。
...
- OMP 子进程内部的 subagent/后台任务与 dsh 侧 jobs 面板的呈现深度——首版按 omp-webui 现状（不呈现）。
